...
void handle_nvme_io_cmd(NVME_COMMAND *nvmeCmd)
{
	...
	unsigned int opc;
	unsigned int entryIteration, reqSlotTag, virtualSliceAddr;
...
/*			xil_printf("OPC = 0x%X\r\n", nvmeIOCmd->OPC);
	...
	switch(opc)
	{
		case IO_NVM_FLUSH:
		{
			...
			for(entryIteration = 0; entryIteration < AVAILABLE_DATA_BUFFER_ENTRY_COUNT; entryIteration++) {
				if(dataBufMapPtr->dataBuf[entryIteration].dirty == DATA_BUF_DIRTY) {
					...
					reqPoolPtr->reqPool[reqSlotTag].nvmeCmdSlotTag = nvmeCmd->cmdSlotTag;
